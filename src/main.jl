...
### User Parameters
################################################################################
target_year = 2025 # either 2025 or 2030
year = 2016 # 2016
CO2_price = 0.04 # This correcponds to 40€/ton of CO2
VOLL = 15000

discrete_model = 1 # set to 1 to solve the discrete investment problem, 0 for the continuous
CRM_included = 0 # set to 1 to include a capacity price
...
selected_nodes = "all" #"all" # "all", "cwe"
erase_nodes_list = ["TR00"] # set to 0 to not erase any node

solve_CRM_from_discrete = 0 # set to 1 if
discrete_solution_path = "/Users/Nicolas/Documents/Mesdocuments/1Travaux/UCL/PhD/Investment_code/InvCode/out/$(target_year)discrete/$(year)_all_discrete_target$(target_year)/solution/"

################################################################################
initial_solution = Nothing
data_path = "/Users/Nicolas/Documents/Mesdocuments/1Travaux/UCL/PhD/Investment_code/data/"
save_path = "out/"
if discrete_model==0 
...
if selected_nodes!="all"
    if selected_nodes=="cwe" # CWE in the report of ENTSO-E (cf. Annex 3: Methodology):
        selected_nodes = ["BE00", "FR00", "NL00", "DE00", "AT00", "LUB1", "LUG1", "LUF1", "LUV1", "DEKF"] # those are the CWE nodes
    end
    println("Reducing the data set.")
    println("Nodes considered in the case study $case_study_name are: $selected_nodes")
    data = keep_subset_nodes(data, selected_nodes)
end
