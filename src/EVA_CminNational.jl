...
"""
function discreteNationalCapacityExpansionModel(data)
    ...
    # for i in generators_new_keys
    #     if i != zone @constraint(EVA_model, [g in generators_new[i]], x_new[i, g] == 0.0) end
    # end
...
function solve_EVAnational_discrete(data, output_path)
    ...
    start_time = time()
    nat_model = discreteNationalCapacityExpansionModel(data)
    for i in data["nodes"]
        if (i in data["generators_new_keys"]) || (i in data["generators_exist_keys"])
            println("solving model of $i")
            # fixed all other variables
            # Set all other countries to 0
            for j in data["generators_new_keys"]
                if j==i
                    for g in data["generators_new"][j]
                        if is_fixed(nat_model[:x_new][j,g])
                            unfix(nat_model[:x_new][j,g])
                        end
                    end
                else
                    for g in data["generators_new"][j]
                        if is_fixed(nat_model[:x_new][j,g])==false
                            fix(nat_model[:x_new][j,g], 0; force = true)
                        end
                    end
                end
            end
            for j in data["generators_exist_keys"]
                if j==i
                    for g in data["generators_exist"][j]
                        if is_fixed(nat_model[:x_exist][j,g])
                            unfix(nat_model[:x_exist][j,g])
                        end
                    end
                else
                    for g in data["generators_exist"][j]
                        if is_fixed(nat_model[:x_exist][j,g])==false
                            fix(nat_model[:x_exist][j,g], 0; force = true)
                        end
                    end
                end
            end
