...
- Date: 2022-09-13
=#
function ProfitMaxModelDiscrete(data, g_type, i, g, price)
...
function compute_maxprofit_discrete(model, data, price)
    ...
    for i in data["generators_new_keys"]
        for g in data["generators_new"][i]
            model_max_profit = ProfitMaxModelDiscrete(data, "New", i, g, price)
    ...
    for i in data["generators_exist_keys"]
        for g in data["generators_exist"][i]
            model_max_profit = ProfitMaxModelDiscrete(data, "Exist", i, g, price)
            set_optimizer_attribute(model_max_profit, "OutputFlag", 0)
            optimize!(model_max_profit)
...
function compute_asclearedprofit_discrete(model, data, price)
    periods = data["periods"]
    generators_exist_keys = data["generators_exist_keys"]
    generators_exist = data["generators_exist"]
    generators_new_keys = data["generators_new_keys"]
    ...
    as_cleared_profit_new = Dict(i=>Dict(
        g => (sum(p_new[i,g,t] * (price[i,t] - deltaT * generators_data["New"][i][g]["OC"]) for t in periods) - x_new[i,g] * generators_data["New"][i][g]["Lumps"] * generators_data["New"][i][g]["IC"])
...
function compute_loc_discrete(model, data, price)
    println("Computing the agents Lost Opportunity Costs...")
    println("Computing the max profit...")
    start_time = time()
    max_profit = compute_maxprofit_discrete(model, data, price)
    elapsed = time() - start_time
    println("solved in $elapsed s.")
    println("Computing the as-cleared profit...")
    start_time = time()
    as_cleared_profit = compute_asclearedprofit_discrete(model, data, price)
    elapsed = time() - start_time
    println("solved in $elapsed s.")
    ...
    TEI = compute_tei(data, value.(model[:x_new]), value.(model[:x_exist]), max_profit["max_profit_x_new"], max_profit["max_profit_x_exist"], loc_new, loc_exist)
